...
function pageFoot({ withReader = false, withPagedReader = false } = {}) {
  const scripts = [];
  if (withReader) scripts.push(`<script src="assets/reader.js" defer></script>`);
  if (withPagedReader) scripts.push(`<script src="assets/reader-pages.js" defer></script>`);
  return `</div>
${scripts.join("\n")}
</body>
</html>`;
...
// Same as pageHead but with an optional body data attribute (used to pick
// reader mode for the paginating reader script).
function pageHeadWithBody(title, bodyAttrs = "") {
  return `<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
...
<div class="page">
  ...
  <section>
    ...
    <p>Pick the reading style you like. Both have the same content — just laid out differently.</p>

    <div class="reader-picker">
      <a class="reader-pick reader-pick--paged" href="read.html">
        <div class="reader-pick-head">
          <span class="reader-pick-eyebrow">Paged</span>
          <h3 class="reader-pick-title">One page at a time</h3>
        </div>
        <p class="reader-pick-desc">Like a PDF reader or Preview app. See one page, click the arrows (or press &larr; &rarr;) to flip. No scrolling.</p>
        <div class="reader-pick-preview reader-pick-preview--paged" aria-hidden="true">
          <div class="rp-card"><span class="rp-lines"></span><span class="rp-lines short"></span><span class="rp-lines"></span></div>
          <div class="rp-controls"><span></span><span class="rp-dot"></span><span></span></div>
        </div>
      </a>
      <a class="reader-pick reader-pick--word" href="pages.html">
        <div class="reader-pick-head">
          <span class="reader-pick-eyebrow">Scroll</span>
          <h3 class="reader-pick-title">Stacked pages, scroll down</h3>
        </div>
        <p class="reader-pick-desc">Like Word's print layout or Google Docs. Pages sit on the desk, you scroll past them — but each page is visible as its own paper card.</p>
        <div class="reader-pick-preview reader-pick-preview--word" aria-hidden="true">
          <div class="rp-card"><span class="rp-lines"></span><span class="rp-lines short"></span></div>
          <div class="rp-card"><span class="rp-lines"></span><span class="rp-lines short"></span></div>
        </div>
      </a>
    </div>

    <p class="reader-extra">
      Other formats:
      <a href="book.html">one long scroll</a> ·
      <a href="PyTB-book.pdf">download PDF</a> ·
      <a href="PyTB-book.docx">download DOCX</a>
    </p>
...
function main() {
  ...
  fs.copyFileSync(path.join(__dirname, "reader-pages.js"),  path.join(ASSETS, "reader-pages.js"));
...
<section class="book-end" data-running-title="">
  ...
  // book.html — long-scroll layout, used as PDF source. The existing
  // reader.js gives it a top progress bar + chapter pill + page chip.
...
${pageFoot({ withReader: true })}`;
  ...
  // pages.html — Word / Google-Docs style: every page card visible, stacked
  // with margins between them. Scroll to read.
  const pagesHtml = `${pageHeadWithBody("PyTB — pages view", `data-reader-mode="word"`)}
<div class="page book-page">
  ${bookBody}
</div>
${pageFoot({ withPagedReader: true })}`;
  fs.writeFileSync(path.join(DIST, "pages.html"), pagesHtml);

  // read.html — Paged reader: one page card at a time, navigated with
  // on-screen buttons + arrow keys + Home / End.
  const readHtml = `${pageHeadWithBody("PyTB — read", `data-reader-mode="paged"`)}
<div class="page book-page">
  ${bookBody}
</div>
${pageFoot({ withPagedReader: true })}`;
  fs.writeFileSync(path.join(DIST, "read.html"), readHtml);
